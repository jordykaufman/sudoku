...
test('what a tap does follows the Tap setting, the Pencil or Place button and rapid mode', () => {
  ...
  assert.equal(tapMakesMark({ tap: 'mark', swapped: true, rapid: false }), false);
  assert.equal(tapMakesMark({ tap: 'place', swapped: false, rapid: false }), false);
  assert.equal(tapMakesMark({ tap: 'place', swapped: true, rapid: false }), true);
  assert.equal(tapMakesMark({ tap: 'place', swapped: false, rapid: true }), true);
});
