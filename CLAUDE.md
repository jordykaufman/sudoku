...
- Practice, from the home menu or a lesson (`src/app/practice.js`): position after position where one technique applies, to find and play. Pattern marks the pattern for practising the move alone, Answer shows the last hint stage, Reset restores the position. A move that is not part of the technique is shown in red and named. Positions come from `data/examples.json` in a different order each time.
